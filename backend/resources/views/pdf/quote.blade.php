...
<body>
    <div class="header"><h1>Presupuesto</h1><div class="muted">{{ $quote->quote_number }} · LumaFlow Studio</div></div>
    <table class="grid"><tr>
        <td><strong>{{ $quote->user->name }}</strong><br>{{ $quote->user->email }}</td>
        <td><strong>Cliente</strong><br>{{ $quote->client->name }}<br>{{ $quote->client->email }}</td>
    </tr></table>
    <p>Emisión: {{ $quote->issue_date->format('d/m/Y') }} @if($quote->valid_until) · Válido hasta: {{ $quote->valid_until->format('d/m/Y') }} @endif</p>
    ...
    <table class="totals">
        <tr><td>Subtotal</td><td class="right">{{ number_format($quote->subtotal, 2, ',', '.') }} €</td></tr>
        <tr><td>IVA ({{ number_format($quote->tax_rate, 2, ',', '.') }}%)</td><td class="right">{{ number_format($quote->tax_amount, 2, ',', '.') }} €</td></tr>
        <tr class="total"><td>Total</td><td class="right">{{ number_format($quote->total, 2, ',', '.') }} €</td></tr>
    </table>
    @if($quote->notes)<div class="notes"><strong>Notas</strong><br>{{ $quote->notes }}</div>@endif
</body>
</html>
