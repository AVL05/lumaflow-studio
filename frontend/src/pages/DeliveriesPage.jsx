...
export function DeliveriesPage() {
  ...
  const resource = usePaginatedResource(deliveriesApi.list, { per_page: 12, sort: 'created_at', direction: 'desc' })
  ...
  return (
    <>
      ...
      {resource.loading ? <DeliverySkeleton /> : resource.items.length === 0 ? (
        <EmptyState
          title="Sin entregas"
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {resource.items.map((delivery) => (
              <DeliveryCard key={delivery.id} delivery={delivery} onEdit={openEdit} onDelete={setDeleting} />
            ))}
          </div>
      ...
      <Modal open={formOpen} title={editing ? 'Editar entrega' : 'Nueva entrega'} onClose={() => setFormOpen(false)}>
        <DeliveryForm
          form={form}
          setForm={setForm}
          clients={clients}
