...
export function AiAssistantPage() {
  ...
  const loadInitialData = useCallback(async () => {
    setError('')
    try {
      const [statusResponse, dashboardResponse, photoResponse, sessionResponse, historyResponse] = await Promise.all([
        aiApi.status(),
  ...
  async function submitChat(event) {
    ...
    setActiveConversation((current) => current ? { ...current, messages: [...current.messages, optimistic] } : { title: content, messages: [optimistic] })

    try {
      const response = await aiApi.chat({ message: content, conversation_id: activeConversation?.id }, { signal: abortRef.current.signal })
  ...
  async function analyzePhoto(event) {
    event.preventDefault()
    await runAiAction('analysis', () => aiApi.analyze({ photo_id: Number(photoId), prompt: analysisPrompt || null }).then(setAnalysis), 'No se pudo analizar la foto.')
  }

  async function generatePreset(payload) {
    await runAiAction('preset', () => aiApi.preset(payload).then(setGeneratedPreset), 'No se pudo generar el preset.')
  }

  async function recommendGear(payload) {
    await runAiAction('gear', () => aiApi.recommendGear(payload).then(setGearRecommendation), 'No se pudo recomendar equipo.')
  }

  async function planSession(payload) {
    await runAiAction('plan', () => aiApi.sessionPlan(payload).then(setSessionPlan), 'No se pudo generar el plan.')
  }
  ...
  async function renameConversation(conversation) {
    ...
    await loadConversations()
    if (activeConversation?.id === conversation.id) setActiveConversation((current) => ({ ...current, title }))
  }
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <Card className="p-5">
            ...
            <form className="mt-4 space-y-4" onSubmit={analyzePhoto}>
              ...
              <Textarea rows="3" value={analysisPrompt} onChange={(event) => setAnalysisPrompt(event.target.value)} placeholder="Objetivo del analisis, estilo buscado o duda concreta" />
              <Button disabled={loading === 'analysis' || !photoId}>{loading === 'analysis' ? 'Analizando...' : 'Analizar fotografia'}</Button>
            </form>
            <div className="mt-5">
        ...
        <div className="grid gap-6 xl:grid-cols-3">
          <PresetGenerator onGenerate={generatePreset} loading={loading === 'preset'} preset={generatedPreset} />
          <GearRecommendation onRecommend={recommendGear} loading={loading === 'gear'} recommendation={gearRecommendation} />
          <SessionPlanner sessions={sessions} onPlan={planSession} loading={loading === 'plan'} plan={sessionPlan} />
        </div>
