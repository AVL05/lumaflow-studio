    ...
    <Card className="p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-500">{delivery.client?.name || 'Sin cliente'} · {delivery.session?.name || 'Sin sesion'}</p>
        </div>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        <Link to={`/app/deliveries/${delivery.id}`}><Button variant="secondary">Detalle</Button></Link>
        <Button variant="secondary" onClick={() => onEdit(delivery)}>Editar</Button>
        <Button variant="danger" onClick={() => onDelete(delivery)}>Eliminar</Button>
      </div>
    </Card>
  )
}
