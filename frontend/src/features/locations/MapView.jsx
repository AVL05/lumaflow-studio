...
export function MapView({
  ...
  useEffect(() => {
    ...
    markers.forEach((marker) => {
      ...
      bounds.push(point);
      leaflet
        .marker(point, { icon: buildMarkerIcon(leaflet, marker.id === selected?.id) })
        .bindPopup(
          `<strong>${escapeHtml(marker.name || "Localizacion")}</strong><br>${escapeHtml(marker.meta || "")}`,
        )
    ...
    if (selected && isValidCoord(selected.latitude, selected.longitude)) {
      ...
      bounds.push(point);
      leaflet
        .marker(point, { icon: buildMarkerIcon(leaflet, true) })
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-lg border border-white/10 bg-[#11100e] ${height}`}
    >
      <div
        ref={mapRef}
      ...
      {selectable ? (
        ...
        >
          ...
          />
          <Button variant="secondary" className="bg-black/70 px-3 py-2 text-xs">
            Ir
          </Button>
        </form>
      ) : null}
