...
export function LocationFilters({ resource }) {
  return (
    <div className="mb-6 grid gap-3 lg:grid-cols-[1fr_140px_140px_140px_140px_120px_130px]">
      ...
      <Select value={resource.filters.access_mode ?? ''} onChange={(event) => resource.updateFilter('access_mode', event.target.value)} options={[{ value: '', label: 'Acceso' }, ...accessModes]} />
      <Select value={resource.filters.favorite ?? ''} onChange={(event) => resource.updateFilter('favorite', event.target.value)} options={[{ value: '', label: 'Todas' }, { value: '1', label: 'Favoritas' }]} />
      <Select value={resource.filters.sort ?? 'created_at'} onChange={(event) => resource.updateFilter('sort', event.target.value)} options={[{ value: 'created_at', label: 'Reciente' }, { value: 'name', label: 'Nombre' }, { value: 'city', label: 'Ciudad' }, { value: 'type', label: 'Tipo' }, { value: 'rating', label: 'Rating' }]} />
    </div>
  )
}
