      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-stone-500">{[location.city, location.country].filter(Boolean).join(', ') || 'Sin ciudad'}</p>
          </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Link to={`/app/locations/${location.id}`}><Button variant="secondary">Detalle</Button></Link>
          <Button variant="secondary" onClick={() => onEdit(location)}>Editar</Button>
          <Button variant="danger" onClick={() => onDelete(location)}>Eliminar</Button>
        </div>
      </div>
    </Card>
  )
