...
export function GearRecommendation({ onRecommend, loading, recommendation }) {
  ...
  return (
    <Card className="p-5">
      ...
      <Button className="mt-4" onClick={() => onRecommend(form)} disabled={loading}>
        {loading ? "Calculando..." : "Recomendar equipo"}
      </Button>
      {recommendation ? (
        <div className="mt-5">
          <JsonViewer value={recommendation.result} />
        </div>
      ) : null}
    </Card>
  );
