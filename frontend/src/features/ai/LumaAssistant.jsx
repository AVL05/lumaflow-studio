...
export function LumaAssistant({ open, onOpen, onClose }) {
  ...
  const context =
    contextByPath.find(([path]) => location.pathname.startsWith(path))?.[1] ?? "LumaFlow";
    ...
    <div className="fixed inset-0 z-[75] bg-black/60 p-4 pt-16 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          <div className="rounded-xl border border-amber-200/15 bg-amber-100/[0.045] p-4">
            ...
            <p className="mt-1 text-xs text-stone-500">
              Luma añadirá esta pantalla a la conversación.
            </p>
          </div>
          <div className="mt-5 grid gap-2">
            {[
              "¿Qué requiere atención aquí?",
              "Ayúdame a preparar el siguiente paso",
              "Resume la situación de este módulo",
            ].map((suggestion) => (
              <button
                key={suggestion}
                type="button"
                onClick={() => setPrompt(suggestion)}
                className="rounded-xl border border-white/10 px-4 py-3 text-left text-sm text-stone-300 hover:border-amber-200/20 hover:bg-white/[0.05]"
              >
                {suggestion}
              </button>
            ))}
        ...
        <form onSubmit={startConversation} className="border-t border-white/10 p-5">
          <label htmlFor="luma-prompt" className="text-sm font-semibold text-stone-200">
            Pregunta a Luma
          </label>
          <textarea
            id="luma-prompt"
            ...
            placeholder="Ejemplo: prepara los próximos pasos para este trabajo"
            className="mt-2 w-full resize-none rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-sm text-stone-100 outline-none placeholder:text-stone-600 focus:border-amber-200/35 focus:ring-2 focus:ring-amber-200/15"
          />
