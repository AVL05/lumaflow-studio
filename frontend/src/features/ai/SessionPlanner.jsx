    ...
    <Card className="p-5">
      ...
      <div className="mt-4 space-y-3">
        ...
        <Textarea
          ...
          placeholder="Objetivos de la sesion"
        />
        <Textarea
          rows="3"
          value={form.constraints}
          onChange={(event) => update("constraints", event.target.value)}
          placeholder="Restricciones: clima, horario, cliente, permisos..."
        />
      </div>
      <Button
        className="mt-4"
        onClick={() => onPlan({ ...form, session_id: Number(form.session_id) })}
        disabled={loading || !form.session_id}
      >
        {loading ? "Planificando..." : "Generar plan"}
      </Button>
      {plan ? (
        <div className="mt-5">
          <JsonViewer value={plan.plan} />
        </div>
      ) : null}
    </Card>
  );
