const variants = {
  primary:
    "bg-amber-100 text-stone-950 shadow-[0_10px_30px_rgba(245,211,141,.16)] hover:bg-stone-50",
  secondary:
    "border border-white/10 bg-white/[0.055] text-stone-100 shadow-[inset_0_1px_0_rgba(255,255,255,.06)] hover:border-amber-200/20 hover:bg-white/[0.09]",
  ghost: "text-stone-400 hover:bg-white/[0.06] hover:text-stone-100",
  danger:
    "border border-red-400/20 bg-red-500/10 text-red-100 hover:border-red-300/30 hover:bg-red-500/20",
};

const base =
  "rounded-lg px-4 py-2 text-sm font-semibold transition duration-200 active:translate-y-px disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-200/60 focus-visible:ring-offset-2 focus-visible:ring-offset-[#090908]";

/**
 * No se fija `type` por defecto a proposito: varios formularios usan el boton
 * sin `type` como submit implicito.
 */
export function Button({ variant = "primary", className = "", ...props }) {
  return <button className={`${base} ${variants[variant]} ${className}`} {...props} />;
}
